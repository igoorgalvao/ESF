...
public class ConstantesARQ {
	...
	private static ResourceBundle bundle = ResourceBundle.getBundle("constantes");
	...
	public static String EMAIL_ERRO_CC = getString("email_erro_cc");
	...
	public static String ENDERECO_SERVIDOR_COM_PATH_SEM_PORTA = getCaminhoAplicacaoSemPorta();

	public static String CONTEXT_PATH = getContextPath();

	// Devolve ip:porta/path
	private static String getCaminhoAplicacao() {
		try {

			InetAddress addr = InetAddress.getLocalHost();

			String path = ConstantesARQ.getRequest().getContextPath();
			String ip = addr.getHostAddress();
			if (ip.trim().equals("10.48.30.98")) {
				ip = ip.replace("10.48.30.98", "noivaembrasilia.com.br");
			}
			Integer porta = ConstantesARQ.getRequest().getLocalPort();
	...
	private static String getCaminhoAplicacaoSemPorta() {
		try {
